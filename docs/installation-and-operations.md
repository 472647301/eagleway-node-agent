...
本文描述目标设计和安全约束。可直接执行的首次部署、本机接口自测、中心联调、源码升级、回滚和下线流程见[运维与服务器自测手册](operations-runbook.md)。
...
6. 不存在证书时，Agent 优先复用宝塔目标站点的 Nginx WebRoot；如果不存在目标域名站点，则在宝塔 vhost 目录创建带所有权标记的最小 ACME 站点，再执行 HTTP-01 签发。Agent 不调用版本相关的 `bt` 菜单编号，也不改写宝塔已有站点配置。
7. 只读引用宝塔证书并写入 Agent 自有配置，不覆盖宝塔已有站点；Certbot 证书安装受管 deploy hook，在续期且被当前 Xray 配置引用时重启运行时。
